...
# Auto-detect: use Parquet if SQLite database doesn't exist (production), otherwise use SQLite (local)
...
def load_data_from_db():
    ...
    try:
        if USE_PARQUET:
            # Production: Load from Parquet files
        ...
        st.error(
            f"Error loading data: {e}. {'Ensure parquet files are in data/ folder' if USE_PARQUET else 'Has the scraper been run?'}")
...
def get_table_data(table_name):
    ...
    if USE_PARQUET:
        try:
            ...
            # If it has stock_id, merge with stocks for ticker names
            if 'stock_id' in df.columns and os.path.exists(f'{DATA_DIR}/stocks.parquet'):
                stocks_df = pd.read_parquet(f'{DATA_DIR}/stocks.parquet')
                df = pd.merge(df, stocks_df[['id', 'ticker']], left_on='stock_id', right_on='id', how='left')
